...
<!-- interpretation -->
The **conditional entropy** values are the smallest for example data 1, where most of the "green" cells are adaject to other "green" ones, and the "yellow" cells are next to other "yellow" cells, and where one category ("green") dominates the entire area.
In example data 2, the proportion of categories is similar, however the cells configuration is less orginised and therefore the **conditional entropy** value is higher than in example data 1.
For the next examples, the **conditional entropy** values grows with both categories proportions and their arrangement.
This is due to a fact that diversity of categories induces configurational complexity.
...
In other words, it measures diversity of values in a co-occurence matrix -- the smaller the diversity, the larger the value of **joint entropy**.
...
Larger values indicate that the cells of the same category are more aggregated, while the smaller values are indication of disaggregation.
...
**Relative mutual information** for the example data orders them from the least aggregated (example data 2, 6), through those with medium aggregation (example data 4, 5), to the most aggregated (example data 1 and 2).
